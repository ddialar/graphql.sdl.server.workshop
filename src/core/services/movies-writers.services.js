...
var logger = log4js.getLogger('movies.writers.services');

import { serverConf }  from '../config';

import * as MoviesService from './movies.services';
import * as WritersService from './writers.services';
import * as CommonService from './common.services';

const { API_REST } = serverConf;
const API_REST_URL = `${API_REST.URL}:${API_REST.PORT}/movies_writers`;
...
const deleteWriterMovies = async (writerId, moviesIds) => {
    try {
        ...
        if (relationshipIdsToBeDeleted) {
            relationshipIdsToBeDeleted = relationshipIdsToBeDeleted.map((movieWriterPairToBeDeleted) => {
                return movieWriterPairToBeDeleted.id;
            });

            return await _deleteMovieWriterRelationships(relationshipIdsToBeDeleted);
        } else {
            logger.warn(`(deleteWriterMovies) - No movie-writer relationships are going to be deleted for the writer with id ${writerId}`); 
            return [];
        }      
    } catch (error) {
