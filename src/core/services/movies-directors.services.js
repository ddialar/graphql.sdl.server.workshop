...
var logger = log4js.getLogger('movies.directors.services');

import { serverConf }  from '../config';

import * as MoviesService from './movies.services';
import * as DirectorsService from './directors.services';
import * as CommonService from './common.services';

const { API_REST } = serverConf;
const API_REST_URL = `${API_REST.URL}:${API_REST.PORT}/movies_directors`;
...
const deleteDirectorMovies = async (directorId, moviesIds) => {
    try {
        ...
        if (relationshipIdsToBeDeleted) {
            relationshipIdsToBeDeleted = relationshipIdsToBeDeleted.map((movieDirectorPairToBeDeleted) => {
