...
var logger = log4js.getLogger('movies.actors.services');

import { serverConf }  from '../config';

import * as MoviesService from './movies.services';
import * as ActorsService from './actors.services';
import * as CommonService from './common.services';

const { API_REST } = serverConf;
const API_REST_URL = `${API_REST.URL}:${API_REST.PORT}/movies_actors`;
...
const deleteActorMovies = async (actorId, moviesIds) => {
    try {
        ...
        if (relationshipIdsToBeDeleted) {
            relationshipIdsToBeDeleted = relationshipIdsToBeDeleted.map((movieActorPairToBeDeleted) => {
